...
Run this checklist from a clean checkout before promoting the `v0.3.0` prerelease. Use synthetic prompts and a disposable named Herdr session. Do not read or copy real transcripts into release evidence.
...
```sh
set -eu
export AGENT_CONTEXT_EVIDENCE_DIR=$(mktemp -d "${TMPDIR:-/tmp}/agent-context-v030-evidence.XXXXXX")
printf '%s\n' "$(git rev-parse HEAD)" > "$AGENT_CONTEXT_EVIDENCE_DIR/release-sha.txt"
test -z "$(git status --porcelain)"
test "$(git rev-parse HEAD)" = "$(git rev-parse origin/main)"
```

Record the existing plugin state, build the same clean SHA, then replace the managed v0.2.0 installation with a source link:
...
jq -e '.result.plugins | length == 1 and .[0].enabled == true and
  ...
  .[0].source.requested_ref == "v0.2.0"' "$evidence/plugin-before.json" >/dev/null
restore_plugin_baseline() {
  herdr plugin unlink ryonakae.agent-context >/dev/null 2>&1 || true
  herdr plugin install ryonakae/herdr-agent-context --ref v0.2.0 --yes >/dev/null
...
herdr plugin list --plugin ryonakae.agent-context --json | jq -e '
  .result.plugins | length == 1 and .[0].enabled == true and
  .[0].version == "0.3.0" and .[0].source.kind == "local"' >/dev/null
```

- [ ] `herdr plugin list --plugin ryonakae.agent-context --json` reports one enabled local `0.3.0` plugin.
...
HERDR_CONFIG_PATH="$AGENT_CONTEXT_EVIDENCE_DIR/herdr-config.toml" \
  herdr --session agent-context-v030
```
...
find "$dist/download" -type f -name 'herdr-agent-context-v0.3.0-*.tar.gz' \
  -exec cp {} "$dist/" \;
if command -v sha256sum >/dev/null 2>&1; then
  (cd "$dist" && sha256sum herdr-agent-context-v0.3.0-*.tar.gz > SHA256SUMS)
else
  (cd "$dist" && shasum -a 256 herdr-agent-context-v0.3.0-*.tar.gz > SHA256SUMS)
fi
sh scripts/verify-release-assets.sh 0.3.0 "$dist"
for target in aarch64-unknown-linux-gnu x86_64-unknown-linux-gnu; do
  mkdir "$dist/$target"
  tar -xzf "$dist/herdr-agent-context-v0.3.0-$target.tar.gz" \
