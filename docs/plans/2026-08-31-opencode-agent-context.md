...
- **D7:** Validate exact/sticky candidates outside ordinary age/count discovery limits, while preserving database membership, unique identity, root-session, cwd, and required-schema checks. Ordinary discovery is bounded to live pane cwd values and compatible root rows. Per database and reconciliation, inspect at most 250 recent root metadata rows by querying with `LIMIT 251`; seeing the overflow row fails the entire ordinary database scan without partial candidates. Canonicalize each distinct stored directory string at most once per scan. Bound SQLite work independently with a progress callback every 1,000 virtual-machine instructions and interrupt after 100 callbacks (approximately 100,000 instructions); interruption is an ordinary read failure. Exact/sticky reads bypass these ordinary limits.
...
**Initial review correction record (2026-08-31):** Independent review of `8518acc..fc3ad19` found four blocking/high contract gaps: required identity primary keys were not validated, ordinary discovery prefiltered canonical cwd aliases, a streamed visible-text update before a later non-display part could leave the fallback fingerprint unchanged, and local sticky read failures lost the OpenCode session-ID naming generation. Focused Red tests reproduced all four. Correction commit `afe5fc3` validates single TEXT identity primary keys, canonical-filters recent root rows before stopping at 25 compatible sessions, hashes all session/message/part state into a privacy-bounded fingerprint, and carries optional session identity through generic failed bindings. OpenCode unit tests pass 33, OpenCode listener tests 9, the complete listener suite 74, and the library suite 232; all-target Clippy, format, and whitespace checks pass.

**Scoped re-review blocker and decision (2026-08-31):** Re-review marked all four original findings resolved but found that the canonical-alias correction removed the SQL directory predicate and `LIMIT`. With fewer than 25 compatible sessions, one unbound pane could synchronously sort, visit, and canonicalize every recent root row on every poll, violating D7 and risking unrelated metadata TTLs. The user selected strengthened option A: a 250-row fail-closed raw discovery cap, per-scan distinct-directory canonicalization, and an independent SQLite virtual-machine instruction budget as now specified in D7.

**Bounded-discovery correction record (2026-08-31):** Synthetic Red tests proved that the prior scan returned a partial target from 251 recent roots and completed an unindexed scan across 100,000 old rows. The correction enables only rusqlite's in-process `hooks` feature, applies the D7 progress handler around ordinary metadata discovery, queries one overflow row, rejects overflow before canonicalization or session-history reads, and memoizes canonical paths by stored directory string for the scan. Exact and sticky reads stay outside the ordinary handler. README documents the 250-row and query-budget limitation. OpenCode unit tests pass 35, OpenCode listener tests 9, the complete library suite 234, and the listener suite 74; all-target Clippy, format, whitespace, release-asset, and release-note checks pass. Task 4 remains incomplete until scoped re-review and the final validation matrix pass.
