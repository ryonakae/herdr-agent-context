...
**Show Pi, Claude Code, and Codex session context in the Herdr sidebar, tab bar, and pane borders**
...
Restart Herdr or reload its configuration. Each supported pane shows its agent, location, Pi session name, Claude session title, or Codex session name, and recent assistant activity.
...
These commands are optional and separate from plugin installation. The plugin never installs integrations or edits Pi, Claude, Codex, or Herdr settings.

An official Pi session path, Claude session ID, or Codex session ID takes priority over local fallback. Codex next uses a structured `codex resume <UUID>` hint, then a valid in-memory binding, then one uniquely new or changed same-cwd rollout after observing the pane. When the official Pi integration is installed, the listener waits for its session path during startup instead of showing a heuristic match. If an authoritative reference is missing or malformed, the listener waits for the exact session instead of showing another transcript. The plugin reports visual metadata only; it never writes inferred paths or IDs back to Herdr.
...
The listener labels each tab from all supported Pi, Claude Code, and Codex panes in it, including background tabs. Components follow visual pane order: top to bottom, then left to right. Pi uses its resolved session name. Claude uses its verified custom or generated title. Codex uses its resolved session name. Untitled and unsupported panes contribute nothing; a tab with no resolved title keeps its baseline label. Agent status and pane focus do not affect label eligibility.
...
Each supported pane uses its own Pi session name, verified Claude title, or Codex session name, bounded to 20 terminal columns. A manual pane rename, including clearing the label, overrides only the current session in that pane. Another session may use its generated label; returning restores the manual override. The listener restores the latest manual pane baseline when the session leaves or pane naming is disabled. Pane overrides do not change tab components.
...
- The listener reads Herdr process metadata and matching local Pi, Claude, or Codex JSONL files. It sends no telemetry or runtime network requests.
...
- Multiple Claude panes in the same project stay empty on a hook-free cold start unless each pane has official or exact UUID evidence. Hook-free Codex fallback stays empty when more than one Codex pane shares the canonical cwd or when multiple compatible candidates are new or changed after pane observation; official IDs and exact UUID resume hints remain eligible.
