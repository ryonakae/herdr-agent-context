...
for target in $TARGETS; do
    tar -czf "$DIST/herdr-agent-context-v0.1.0-${target}.tar.gz" \
        -C "$STAGING" herdr-agent-context LICENSE
done
checksums "$DIST"
"$VERIFY" 0.1.0 "$DIST" >/dev/null

cp -R "$DIST" "$TMP/missing"
rm "$TMP/missing/herdr-agent-context-v0.1.0-aarch64-apple-darwin.tar.gz"
if "$VERIFY" 0.1.0 "$TMP/missing" >/dev/null 2>&1; then
    echo "release test: missing archive unexpectedly passed" >&2
    exit 1
fi

cp -R "$DIST" "$TMP/extra"
cp "$DIST/herdr-agent-context-v0.1.0-aarch64-apple-darwin.tar.gz" \
    "$TMP/extra/herdr-agent-context-v0.1.0-extra-target.tar.gz"
checksums "$TMP/extra"
if "$VERIFY" 0.1.0 "$TMP/extra" >/dev/null 2>&1; then
    echo "release test: extra archive unexpectedly passed" >&2
    exit 1
fi

cp -R "$DIST" "$TMP/corrupt"
printf 'corruption' >>"$TMP/corrupt/herdr-agent-context-v0.1.0-x86_64-apple-darwin.tar.gz"
if "$VERIFY" 0.1.0 "$TMP/corrupt" >/dev/null 2>&1; then
...
tar -czf "$TMP/contents/herdr-agent-context-v0.1.0-aarch64-apple-darwin.tar.gz" \
    -C "$STAGING" herdr-agent-context LICENSE -C "$TMP/extra-file" README
checksums "$TMP/contents"
if "$VERIFY" 0.1.0 "$TMP/contents" >/dev/null 2>&1; then
