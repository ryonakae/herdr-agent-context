...
for target in $TARGETS; do
    tar -czf "$DIST/herdr-agent-context-v${VERSION}-${target}.tar.gz" \
        -C "$STAGING" herdr-agent-context LICENSE
done
checksums "$DIST"
"$VERIFY" "$VERSION" "$DIST" >/dev/null

cp -R "$DIST" "$TMP/missing"
rm "$TMP/missing/herdr-agent-context-v${VERSION}-aarch64-apple-darwin.tar.gz"
if "$VERIFY" "$VERSION" "$TMP/missing" >/dev/null 2>&1; then
    echo "release test: missing archive unexpectedly passed" >&2
    exit 1
fi

cp -R "$DIST" "$TMP/extra"
cp "$DIST/herdr-agent-context-v${VERSION}-aarch64-apple-darwin.tar.gz" \
    "$TMP/extra/herdr-agent-context-v${VERSION}-extra-target.tar.gz"
checksums "$TMP/extra"
if "$VERIFY" "$VERSION" "$TMP/extra" >/dev/null 2>&1; then
    echo "release test: extra archive unexpectedly passed" >&2
    exit 1
fi

cp -R "$DIST" "$TMP/corrupt"
printf 'corruption' >>"$TMP/corrupt/herdr-agent-context-v${VERSION}-x86_64-apple-darwin.tar.gz"
if "$VERIFY" "$VERSION" "$TMP/corrupt" >/dev/null 2>&1; then
...
tar -czf "$TMP/contents/herdr-agent-context-v${VERSION}-aarch64-apple-darwin.tar.gz" \
    -C "$STAGING" herdr-agent-context LICENSE -C "$TMP/extra-file" README
checksums "$TMP/contents"
if "$VERIFY" "$VERSION" "$TMP/contents" >/dev/null 2>&1; then
...
tar -czf "$TMP/symlink/herdr-agent-context-v${VERSION}-aarch64-apple-darwin.tar.gz" \
    -C "$TMP/symlink-stage" herdr-agent-context LICENSE
checksums "$TMP/symlink"
if "$VERIFY" "$VERSION" "$TMP/symlink" >/dev/null 2>&1; then
...
tar -czf "$TMP/license-symlink/herdr-agent-context-v${VERSION}-aarch64-apple-darwin.tar.gz" \
    -C "$TMP/license-symlink-stage" herdr-agent-context LICENSE
checksums "$TMP/license-symlink"
if "$VERIFY" "$VERSION" "$TMP/license-symlink" >/dev/null 2>&1; then
...
tar -czf "$TMP/hardlink/herdr-agent-context-v${VERSION}-aarch64-apple-darwin.tar.gz" \
    -C "$TMP/hardlink-stage" LICENSE herdr-agent-context
checksums "$TMP/hardlink"
if "$VERIFY" "$VERSION" "$TMP/hardlink" >/dev/null 2>&1; then
    echo "release test: hardlink binary unexpectedly passed" >&2
    exit 1
fi

"$ROOT/scripts/verify-version.sh" "v$VERSION" "$ROOT" >/dev/null
if "$ROOT/scripts/verify-version.sh" "$VERSION" "$ROOT" >/dev/null 2>&1; then
    echo "release test: bare version unexpectedly passed" >&2
    exit 1
fi
mkdir "$TMP/version"
cp "$ROOT/Cargo.toml" "$TMP/version/Cargo.toml"
cp "$ROOT/Cargo.lock" "$TMP/version/Cargo.lock"
cp "$ROOT/herdr-plugin.toml" "$TMP/version/herdr-plugin.toml"
cp "$ROOT/CHANGELOG.md" "$TMP/version/CHANGELOG.md"
if "$ROOT/scripts/verify-version.sh" "v${VERSION}.1" "$TMP/version" >/dev/null 2>&1; then
