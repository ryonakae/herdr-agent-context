...
for target in $TARGETS; do
    tar -czf "$DIST/herdr-agent-context-v0.4.0-${target}.tar.gz" \
        -C "$STAGING" herdr-agent-context LICENSE
done
checksums "$DIST"
"$VERIFY" 0.4.0 "$DIST" >/dev/null

cp -R "$DIST" "$TMP/missing"
rm "$TMP/missing/herdr-agent-context-v0.4.0-aarch64-apple-darwin.tar.gz"
if "$VERIFY" 0.4.0 "$TMP/missing" >/dev/null 2>&1; then
    echo "release test: missing archive unexpectedly passed" >&2
    exit 1
fi

cp -R "$DIST" "$TMP/extra"
cp "$DIST/herdr-agent-context-v0.4.0-aarch64-apple-darwin.tar.gz" \
    "$TMP/extra/herdr-agent-context-v0.4.0-extra-target.tar.gz"
checksums "$TMP/extra"
if "$VERIFY" 0.4.0 "$TMP/extra" >/dev/null 2>&1; then
    echo "release test: extra archive unexpectedly passed" >&2
    exit 1
fi

cp -R "$DIST" "$TMP/corrupt"
printf 'corruption' >>"$TMP/corrupt/herdr-agent-context-v0.4.0-x86_64-apple-darwin.tar.gz"
if "$VERIFY" 0.4.0 "$TMP/corrupt" >/dev/null 2>&1; then
...
tar -czf "$TMP/contents/herdr-agent-context-v0.4.0-aarch64-apple-darwin.tar.gz" \
    -C "$STAGING" herdr-agent-context LICENSE -C "$TMP/extra-file" README
checksums "$TMP/contents"
if "$VERIFY" 0.4.0 "$TMP/contents" >/dev/null 2>&1; then
...
tar -czf "$TMP/symlink/herdr-agent-context-v0.4.0-aarch64-apple-darwin.tar.gz" \
    -C "$TMP/symlink-stage" herdr-agent-context LICENSE
checksums "$TMP/symlink"
if "$VERIFY" 0.4.0 "$TMP/symlink" >/dev/null 2>&1; then
...
tar -czf "$TMP/license-symlink/herdr-agent-context-v0.4.0-aarch64-apple-darwin.tar.gz" \
    -C "$TMP/license-symlink-stage" herdr-agent-context LICENSE
checksums "$TMP/license-symlink"
if "$VERIFY" 0.4.0 "$TMP/license-symlink" >/dev/null 2>&1; then
...
tar -czf "$TMP/hardlink/herdr-agent-context-v0.4.0-aarch64-apple-darwin.tar.gz" \
    -C "$TMP/hardlink-stage" LICENSE herdr-agent-context
checksums "$TMP/hardlink"
if "$VERIFY" 0.4.0 "$TMP/hardlink" >/dev/null 2>&1; then
    echo "release test: hardlink binary unexpectedly passed" >&2
    exit 1
fi

"$ROOT/scripts/verify-version.sh" v0.4.0 "$ROOT" >/dev/null
if "$ROOT/scripts/verify-version.sh" 0.4.0 "$ROOT" >/dev/null 2>&1; then
